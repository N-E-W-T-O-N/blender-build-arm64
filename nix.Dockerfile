...
RUN nix-env -iA \
    ...
    nixpkgs.ispc                               \      
    nixpkgs.brotli                             \
    nixpkgs.jemalloc    nixpkgs.onetbb nixpkgs.oneDNN  nixpkgs.busybox                      \
    \
...
#RUN python3.11 -m pip install --no-cache-dir numpy charset-normalizer urllib3 zstandard requests idna certifi alembic mako jinja2
#ENV PYTHONPATH=/root/.nix-profile/lib/python3.11/site-packages:${PYTHONPATH}

#RUN python3.11 -m pip install numpy charset-normalizer urllib3 zstandard requests idna certifi alembic mako jinja2
