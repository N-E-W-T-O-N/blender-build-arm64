...
cmake .. -GNinja -DCMAKE_INSTALL_PREFIX=/usr/local -DCMAKE_CXX_FLAGS="-Wno-error=stringop-overflow " -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON -DTBB_ROOT=/usr/local -DEMBREE_INCLUDE_DIR=/usr/local/include/embree4 \
 -DEMBREE_LIBRARY=/usr/local/lib/libembree4.so -DMaterialX_ROOT=/usr/local -DPXR_BUILD_USD_TOOLS=ON -DPXR_ENABLE_GL_SUPPORT=ON -DPXR_BUILD_EMBREE_PLUGIN=ON -DPXR_BUILD_ALEMBIC_PLUGIN=ON -DPXR_ENABLE_PYTHON_SUPPORT=ON \
