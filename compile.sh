...
# FILE="/blender-git/blender/intern/cycles/scene/colorspace.cpp"

# if [ -f "$FILE" ]; then
#   sed -i 's|device_processor->apply(desc);|//    device_processor->apply(desc);\ndevice_processor->apply(const_cast<OpenColorIO_v2_1::PackedImageDesc&>(desc));|' "$FILE"
# fi
...
make ninja  BUILD_CMAKE_ARGS="-DWITH_CYCLES_DEVICE_CUDA=OFF -DWITH_CYCLES_DEVICE_HIP=OFF -DWITH_CYCLES_DEVICE_OPTIX=OFF -DWITH_CYCLES_CUDA_BINARIES=OFF -DSDL2_INCLUDE_DIRS=/usr/include/SDL2/  -DPYTHON_NUMPY_INCLUDE_DIRS=/usr/local/lib/python3.11/site-packages/numpy/_core/include -DOPENCOLORIO_OPENCOLORIO_LIBRARY=/usr/local/lib/aarch64-linux-gnu/libOpenColorIO.so  -DWITH_INSTALL_PORTABLE=ON"
