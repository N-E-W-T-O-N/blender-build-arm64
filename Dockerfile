...
RUN echo "Building TBB" && \
    git clone https://github.com/uxlfoundation/oneTBB --branch v2022.2.0 /tmp/build_linux/tbb && \
    cd /tmp/build_linux/tbb && mkdir -p build && cd build && \
...
RUN echo "Add Vulkan Header" &&\
    git clone --depth=1 -b v1.3.296 https://github.com/KhronosGroup/Vulkan-Headers.git  /tmp/vulkan && cd /tmp/vulkan && \ 
...
RUN echo "Build OPENCOLORIO" && \
    git clone --branch  v2.5.0 --depth=1 \
      https://github.com/AcademySoftwareFoundation/OpenColorIO /tmp/build_linux/ocio && \
    mkdir -p /tmp/build_linux/oiio/build && cd /tmp/build_linux/oiio/build && \
    # Configure
    ...
    -DOCIO_BUILD_DOCS=OFF \
     && \
       cmake --build . && \
       cmake --install . && \
       echo "Done Installing OpenColorIO."
...
# 8 Denoise 
# Build OIDN
#https://github.com/RenderKit/oidn/releases/download/v2.3.3/oidn-2.3.3.src.zip
#git clone --branch v2.3.3 --depth=1 https://github.com/RenderKit/oidn.git /tmp/build_linux/oidn && \
#https://github.com/ispc/ispc/releases/download/v1.28.2/ispc-v1.28.2-linux.aarch64.tar.gz -O /tmp/ispc.tar.gz && \
#tar -xvf /tmp/ispc.tar.gz -C /opt/ispc --strip-components=1 && \
#git clone https://github.com/RenderKit/oidn-weights weights && \
#GIT_DISCOVERY_ACROSS_FILESYSTEM=1 git clone https://github.com/RenderKit/mkl-dnn -b v2.2.4 external/mkl-dnn && \ 
        
RUN echo "Build Image Denoiser Oidn by Intel" && \
    wget https://github.com/RenderKit/oidn/releases/download/v2.3.3/oidn-2.3.3.src.tar.gz -O oidn.tar.gz && \
    echo "Done Downloading Oidn" && mkdir /tmp/build_linux/oidn && \
    tar -xvf oidn.tar.gz -C /tmp/build_linux/oidn --strip-components=1 && rm -rf oidn.tar.gz && \
    cd /tmp/build_linux/oidn && \
    mkdir -p build && cd build && \
    cmake ..  --trace-expand -GNinja \
    ...
    echo "DOne installing Image Denoiser Oidn"
...
RUN echo "Building OpenPGL (direct CMake build)" && \
    git clone --single-branch https://github.com/RenderKit/openpgl --branch v0.7.1 /tmp/build_linux/openpgl && \
    cd /tmp/build_linux/openpgl && mkdir -p build && cd build && \
    cmake .. \
      -DCMAKE_BUILD_TYPE=Release \
      -DCMAKE_INSTALL_PREFIX=/usr/local \
...
RUN echo "Building OpenSubdiv" && \
    git clone https://github.com/PixarAnimationStudios/OpenSubdiv.git --depth=1 /tmp/build_linux/OpenSubdiv && \
    cd /tmp/build_linux/OpenSubdiv && mkdir -p build && cd build && \
    cmake .. \
      -G Ninja \
      -DCMAKE_BUILD_TYPE=Release \
      -DCMAKE_INSTALL_PREFIX=/usr/local \
...
RUN echo "Building Embree 4" && \
    git clone --branch v4.4.0 --single-branch https://github.com/RenderKit/embree.git /tmp/build_linux/embree && \
    cd /tmp/build_linux/embree && mkdir -p build && cd build && \
    cmake .. \
    -DCMAKE_BUILD_TYPE=Release \
    -DCMAKE_CXX_FLAGS="-Wno-error=stringop-overflow" \
...
RUN echo "Building Embree 3" && \
    git clone --branch v4.4.0 --single-branch https://github.com/RenderKit/embree.git /tmp/build_linux/embree3 && \
    cd /tmp/build_linux/embree3 && mkdir -p build && cd build && \
    cmake .. \
    -DCMAKE_BUILD_TYPE=Release \
    -DCMAKE_INSTALL_PREFIX=/opt/embree3 \
    -DCMAKE_CXX_FLAGS="-Wno-error=stringop-overflow" \
    -DEMBREE_TUTORIALS=OFF \
    -DEMBREE_SYCL_SUPPORT=OFF \
    -DEMBREE_ISPC_SUPPORT=ON \
...
RUN echo "Building Alembic" && \
    git clone https://github.com/alembic/alembic/ -b 1.8.8 --depth=1 /tmp/build_linux/alembic && \
    cd /tmp/build_linux/alembic && mkdir -p build && cd build && \
    cmake .. \
      -DCMAKE_BUILD_TYPE=Release \
      -DCMAKE_INSTALL_PREFIX=/usr/local \
      -DUSE_TESTS=OFF \
...
RUN echo "Building PipeWire v1.4.8" && \
    git clone https://github.com/PipeWire/pipewire --branch 1.4.8 --depth=1 /tmp/build_linux/pipewire && \
    cd /tmp/build_linux/pipewire && mkdir -p build && cd build && \
    meson setup . .. --buildtype=release -Dprefix=/usr/local --backend=ninja  -Dtests=disabled -Ddocs=disabled -Dexamples=disabled \
...
RUN echo "Building OpenUSD v25.05.01" && \
    git clone https://github.com/PixarAnimationStudios/OpenUSD/ -b v25.08 --depth=1 /tmp/build_linux/OpenUSD && \
    cd /tmp/build_linux/OpenUSD && mkdir -p build && cd build && \
    cmake .. \
      -DCMAKE_CXX_FLAGS="-Wno-error=stringop-overflow" \
      -DCMAKE_BUILD_TYPE=Release \
      -DCMAKE_INSTALL_PREFIX=/usr/local \
